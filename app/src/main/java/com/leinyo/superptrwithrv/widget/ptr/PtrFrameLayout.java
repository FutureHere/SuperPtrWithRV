...
public class PtrFrameLayout extends ViewGroup {
    ...
    private PerformRefreshCompleteDelay mPerformRefreshCompleteDelay = new PerformRefreshCompleteDelay(this);
    ...
    static class PerformRefreshCompleteDelay implements Runnable {
        private final WeakReference<PtrFrameLayout> mView;

        PerformRefreshCompleteDelay(PtrFrameLayout view) {
        ...
        public void run() {
            if (mView.get() != null) {
                mView.get().performRefreshComplete();
            }
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isDebug()) {
            Timber.d(LOG_TAG, "onMeasure frame: width: %s, height: %s, padding: %s %s %s %s",
        ...
        if (mContent != null) {
            ...
            if (isDebug()) {
                ...
                Timber.d(LOG_TAG, "onMeasure content, width: %s, height: %s, margin: %s %s %s %s",
                        getMeasuredWidth(), getMeasuredHeight(),
                        lp.leftMargin, lp.topMargin, lp.rightMargin, lp.bottomMargin);
                Timber.d(LOG_TAG, "onMeasure, currentPos: %s, lastPos: %s, top: %s",
    ...
    private void layoutChildren() {
        ...
        if (mHeaderView != null) {
            ...
            if (isDebug()) {
                Timber.d(LOG_TAG, "onLayout header: %s %s %s %s", left, top, right, bottom);
            }
        }
        if (mContent != null) {
            if (isPinContent()) {
            ...
            if (isDebug()) {
                Timber.d(LOG_TAG, "onLayout content: %s %s %s %s", left, top, right, bottom);
            }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (DEBUG) {
                    ...
                    Timber.v(LOG_TAG, "ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s", offsetY, mPtrIndicator.getCurrentPosY(), moveUp, canMoveUp, moveDown, canMoveDown);
                }
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (mPtrIndicator.hasLeftStartPosition()) {
                    if (DEBUG) {
                        Timber.d(LOG_TAG, "call onRelease when user release");
                    }
                    onRelease(false);
    ...
    private void movePos(float deltaY) {
        ...
        if ((deltaY < 0 && mPtrIndicator.isInStartPosition())) {
            if (DEBUG) {
                Timber.e(LOG_TAG, String.format("has reached the top"));
            }
            return;
        }
        ...
        if (mPtrIndicator.willOverTop(to)) {
            if (DEBUG) {
                Timber.e(LOG_TAG, String.format("over top"));
            }
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT) ||
            ...
            if (DEBUG) {
                Timber.i(LOG_TAG, "PtrUIHandler: onUIRefreshPrepare, mFlag %s", mFlag);
            }
        }
        ...
        if (DEBUG) {
            Timber.v(LOG_TAG, "updatePos: change: %s, current: %s last: %s, top: %s, headerHeight: %s",
    ...
    public void setRefreshCompleteHook(PtrUIHandlerHook hook) {
        ...
        hook.setResumeAction(new Runnable() {
            ...
            public void run() {
                if (DEBUG) {
                    Timber.d(LOG_TAG, "mRefreshCompleteHook resume.");
                }
    ...
    private void performRefresh() {
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            ...
            if (DEBUG) {
                Timber.i(LOG_TAG, "PtrUIHandler: onUIRefreshBegin");
            }
        }
        if (mPtrHandler != null) {
    ...
    private boolean tryToNotifyReset() {
        if ((mStatus == PTR_STATUS_COMPLETE || mStatus == PTR_STATUS_PREPARE) && mPtrIndicator.isInStartPosition()) {
            if (mPtrUIHandlerHolder.hasHandler()) {
                ...
                if (DEBUG) {
                    Timber.i(LOG_TAG, "PtrUIHandler: onUIReset");
                }
            }
    ...
    protected void onPtrScrollAbort() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
                Timber.d(LOG_TAG, "call onRelease after scroll abort");
            }
            onRelease(true);
        }
    }
    ...
    protected void onPtrScrollFinish() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
                Timber.d(LOG_TAG, "call onRelease after scroll finish");
            }
            onRelease(true);
        }
    }
    ...
    final public void refreshComplete() {
        if (DEBUG) {
            Timber.i(LOG_TAG, "refreshComplete");
        }
        ...
        if (delay <= 0) {
            if (DEBUG) {
                Timber.d(LOG_TAG, "performRefreshComplete at once");
            }
            ...
            if (DEBUG) {
                Timber.d(LOG_TAG, "performRefreshComplete after delay: %s", delay);
            }
        }
    }
    ...
    private void performRefreshComplete() {
        ...
        if (mScrollChecker.mIsRunning && isAutoRefresh()) {
            // do nothing
            if (DEBUG) {
                Timber.d(LOG_TAG, "performRefreshComplete do nothing, scrolling: %s, auto refresh: %s",
    ...
    private void notifyUIRefreshComplete(boolean ignoreHook) {
        ...
        if (mPtrIndicator.hasLeftStartPosition() && !ignoreHook && mRefreshCompleteHook != null) {
            if (DEBUG) {
                Timber.d(LOG_TAG, "notifyUIRefreshComplete mRefreshCompleteHook run.");
            }
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            if (DEBUG) {
                Timber.i(LOG_TAG, "PtrUIHandler: onUIRefreshComplete");
            }
    ...
    private void sendCancelEvent() {
        if (DEBUG) {
            Timber.d(LOG_TAG, "send cancel event");
        }
    ...
    private void sendDownEvent() {
        if (DEBUG) {
            Timber.d(LOG_TAG, "send down event");
        }
    ...
    class ScrollChecker implements Runnable {
        ...
        private void finish() {
            if (DEBUG) {
                Timber.v(LOG_TAG, "finish, currentPos:%s", mPtrIndicator.getCurrentPosY());
            }
            reset();
            onPtrScrollFinish();
        }
        ...
        public void tryToScrollTo(int to, int duration) {
            ...
            if (DEBUG) {
                Timber.d(LOG_TAG, "tryToScrollTo: start: %s, distance:%s, to:%s", mStart, distance, to);
            }
            removeCallbacks(this);

package com.leinyo.superptrwithrv.widget.ptr.header;
...
public class StoreHouseHeader extends View implements PtrUIHandler {
    ...
    private void initView() {
        mLineWidth = CommonUtils.dpTopx(getContext(),1);
        mDropHeight = CommonUtils.dpTopx(getContext(),40);
        mHorizontalRandomness = CommonUtils.getScreenWidthpx(getContext()) / 2;
    }
    ...
    private int getTopOffset() {
        return getPaddingTop() + CommonUtils.dpTopx(getContext(),10);
    }

    private int getBottomOffset() {
        return getPaddingBottom() + CommonUtils.dpTopx(getContext(),10);
    }
    ...
    public void initWithPointList(ArrayList<float[]> pointList) {
        ...
        for (int i = 0; i < pointList.size(); i++) {
            ...
            PointF startPoint = new PointF(CommonUtils.dpTopx(getContext(),line[0]) * mScale, CommonUtils.dpTopx(getContext(),line[1]) * mScale);
            PointF endPoint = new PointF(CommonUtils.dpTopx(getContext(),line[2]) * mScale,CommonUtils.dpTopx(getContext(),line[4]) * mScale);
